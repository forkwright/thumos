...
pub(crate) fn ah(irk: &[u8; 16], prand: &[u8; 3]) -> [u8; 3] {
    ...
    // INVARIANT: Aes128::new_from_slice only fails on a wrong-length key;
    // irk is &[u8; 16] by the type system.
    let cipher = Aes128::new_from_slice(irk).expect("AES-128 key is 16 bytes");
