...
//!   `grants`). A verified response proves the responder knows the signed
...
pub fn encode_authenticated_request(
    ...
    let payload = postcard::to_allocvec(&wrapped).map_err(CoreError::Encode)?;
...
pub fn decode_authenticated_response(bytes: &[u8]) -> Result<AuthenticatedResponse> {
    let frame = Envelope::decode(bytes).map_err(CoreError::Envelope)?;
