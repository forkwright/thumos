...
impl Console {
    ...
    pub fn receive_byte(&mut self, byte: u8) {
        match byte {
            ...
            0x7F | 0x08 => {
                if self.line_len > 0 {
                    self.line_len -= 1;
                    self.serial.write_str("\x08 \x08").ok(); // Erase character
                }
            }
            // Printable
            0x20..=0x7E => {
    ...
    fn execute(&mut self, cmd: &str) {
        ...
        match parts[0] {
            ...
            _ => {
                write!(self.serial, "unknown command: {}\r\n", parts[0]).ok();
                self.serial.write_str("type 'help' for commands\r\n").ok();
            }
        }
    }
    ...
    fn cmd_help(&mut self) {
        ...
        self.serial
            ...
            .ok();
        self.serial.write_str("  ps      — show processes\r\n").ok();
        self.serial
    ...
    fn cmd_mem(&mut self) {
        ...
        write!(self.serial, "pages: {} free ({} MB)\r\n", free, free_mb).ok();
        write!(
            self.serial,
            "heap: {} / {} bytes ({:.1}%)\r\n",
            heap_used,
    ...
    fn cmd_ps(&mut self) {
        write!(self.serial, "PID {} running\r\n", process::current_pid()).ok();
    }

    fn cmd_version(&mut self) {
        self.serial.write_str("thumos/pyknosis v0.1.0\r\n").ok();
        self.serial
