...
//! ## Font
//!
//! The 8x16 bitmap font table and its dimension constants are canonical in
//! `eidolon-core` (#545); this module and `crates/eidolon/src/font.rs` both
//! alias them rather than each keeping its own copy. `draw_char`/`draw_str`/
//! `draw_char_scaled` stay kernel-local -- they render into this module's raw
...
// ---------------------------------------------------------------------------
// Font constants (canonical table lives in eidolon-core, #545 -- see the
// aliasing WHY note above the display-layout-constants block)
// ---------------------------------------------------------------------------

/// Width of each character cell in pixels.
pub(crate) const CHAR_WIDTH: u16 = eidolon_core::CHAR_WIDTH;

/// Height of each character cell in pixels.
pub(crate) const CHAR_HEIGHT: u16 = eidolon_core::CHAR_HEIGHT;
...
/// the leftmost pixel. Canonical copy lives in [`eidolon_core::FONT_DATA`]
/// (#545); both this module and `crates/eidolon/src/font.rs` alias it.
///
/// `pub(crate)` because `screen_home` uses it for scaled text rendering.
pub(crate) static FONT_DATA: [[u8; 16]; FONT_CHAR_COUNT] = eidolon_core::FONT_DATA;
...
///
/// Matches `crates/eidolon/src/color.rs`'s `Rgb565` constants. Kept as raw
/// `u16` here, not because eidolon requires `std` (it has been `#![no_std]`
...
///
/// Duplicated here, not converged: haphe has been `#![no_std]` with zero
/// dependencies since its first commit, so linkability was never what kept
/// this separate (`crates/thumos/Cargo.toml` already path-links it as a
/// dev-dependency for host tests, #615). Only the digit / star / hash /
/// d-pad prefix (`Num0..=Right`, discriminants 0-15) is a genuine shared
/// vocabulary with `haphe::input::Key` — the same physical
