...
//! + payload region and is the authenticity gate. [`PROVISION_PUBLIC_KEY`] is
//!   generated by `build.rs` (#869): a production build must be given one
//!   through `THUMOS_PROVISION_KEY_PUB` or it fails to build, and any other
//!   build has `None` -- no anchor, so every bundle is refused. #869 still owns
//!   command identity, freshness, and replay semantics.
//!
//! ## Provisioning flow
//!
...
// PROVISION_PUBLIC_KEY / PROVISION_KEY_IS_PRODUCTION, generated by build.rs
// (#869). A plain comment, not a doc comment: `include!` is not an item and
// cannot carry one.
//
// `None` means this build carries no provisioning anchor and refuses every
// bundle. That is the non-production state, and it is structural rather than
// enforced: there is nothing to verify against, so no path can present a
// bundle as operator-authenticated. It replaced RFC 8032 test vector 2, whose
// private half is published in the RFC -- a key that authenticated anybody
// while the doc beside it described operator offline custody.
//
// build.rs also refuses a provisioning anchor equal to the boot anchor:
// kernel-image authenticity and provisioning-bundle authenticity are separate
// trust domains, and one key serving both silently merges them.
include!(concat!(env!("OUT_DIR"), "/provision_key.rs"));
...
pub enum ProvisionError {
    ...
    SignatureInvalid,
    /// This build carries no provisioning trust anchor, so nothing can be
    /// authenticated. Distinct from [`Self::SignatureInvalid`], which means a
    /// signature was checked and did not match -- here none was checked, and a
    /// caller must not report the bundle as having failed verification.
    NoTrustAnchor,
...
impl fmt::Display for ProvisionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NoTrustAnchor => write!(f, "no provisioning trust anchor in this build"),
...
pub(crate) struct Provisioner {
    ...
    /// Ed25519 public key used to verify bundle signatures. `None` when this
    /// build carries no provisioning anchor, in which case every bundle is
    /// refused. Outside tests this is `PROVISION_PUBLIC_KEY`; test-injectable
    /// via [`Provisioner::new_with_key`] to exercise verification against a
    /// locally generated keypair.
    provision_public_key: Option<[u8; secure_boot::PUBLIC_KEY_LEN]>,
}

impl Provisioner {
    /// Create a new provisioner in the [`ProvisionState::Waiting`] state.
    ///
    /// Verifies bundle signatures against [`PROVISION_PUBLIC_KEY`], and refuses
    /// every bundle when this build has none.
    #[must_use]
    ...
    pub(crate) fn new_with_key(provision_public_key: [u8; secure_boot::PUBLIC_KEY_LEN]) -> Self {
        Self {
            ...
            payload_len: None,
            bundle: None,
            provision_public_key: Some(provision_public_key),
        }
    }
    ...
    fn try_finalize(&self, payload_len: usize) -> Result<ProvisionBundle, ProvisionError> {
        ...
        // Verify the Ed25519 signature over the same region: the authenticity
        // gate. A build with no anchor refuses here and says so distinctly --
        // reporting SignatureInvalid would claim a signature was checked and
        // rejected, when none was checked at all (#869).
        let anchor = self
            .provision_public_key
            .ok_or(ProvisionError::NoTrustAnchor)?;
        let mut signature = [0u8; SIGNATURE_LEN];
        signature.copy_from_slice(&self.buffer[signature_start..signature_start + SIGNATURE_LEN]);
        secure_boot::verify_message_signature(data_region, &signature, &anchor)
...
/// [`Provisioner::try_finalize`] verifies). A production build is given its
/// trust anchor through `THUMOS_PROVISION_KEY_PUB` or fails to build (#869);
/// the matching signing key stays under operator custody and never appears
/// here. This function never touches
...
mod tests {
    ...
    #[test]
    fn a_build_with_no_anchor_refuses_a_correctly_signed_bundle() {
        // The non-production state, exercised through the real entry points
        // rather than by constructing the struct by hand. The bundle here is
        // validly signed -- by somebody -- and must still be refused, because
        // this build trusts nobody.
        ...
        assert_eq!(
            prov.finalize(),
            Err(ProvisionError::NoTrustAnchor),
            "a bundle correctly signed by someone must still be refused when this \
             build carries no anchor"
        );
    }

    #[test]
    fn no_anchor_is_distinguishable_from_a_rejected_signature() {
        // A caller reporting SignatureInvalid for a missing anchor would be
        // claiming a verification happened. An operator would then hunt a
        // corrupted bundle instead of a build that trusts nobody -- so the two
        // must not be the same value.
        let bundle = provision_bundle_with_cross_signing();

        let no_anchor = {
            let mut prov = Provisioner::new();
            prov.receive_chunk(&encode_bundle_signed(&bundle));
            prov.finalize()
        };
        let wrong_signer = {
            // Trusts the harness key, but the bundle is signed by a different one.
            let mut prov = harness_provisioner();
            prov.receive_chunk(&encode_bundle_with(
                &bundle,
        ...
        assert_eq!(no_anchor, Err(ProvisionError::NoTrustAnchor));
        assert_eq!(wrong_signer, Err(ProvisionError::SignatureInvalid));
        assert_ne!(no_anchor, wrong_signer);
    }

    #[test]
    // WHY the allow: these constants are generated per build, so clippy sees a
    // fixed value for THIS compilation and calls the assertion constant. Pinning
    // that per-build value is exactly the test's job, and it stays a runtime
    // #[test] rather than a const assert so it remains a counted, individually
    // reportable entry in the suite -- the same shape secure_boot.rs uses for the
    // boot anchor.
    #[allow(clippy::assertions_on_constants)]
    fn the_rfc_test_vector_is_not_this_build_anchor() {
        // The exact key this replaced: RFC 8032 test vector 2, whose private
        // half is published in the RFC. build.rs refuses it for either anchor
        // in every configuration; this pins that no build ships it embedded.
        const RFC8032_TEST2: [u8; secure_boot::PUBLIC_KEY_LEN] = [
        ...
        assert_ne!(
            PROVISION_PUBLIC_KEY,
            Some(RFC8032_TEST2),
            "the provisioning anchor must never be a published test vector"
        );
    }
    ...
    // boot anchor.
    #[allow(clippy::assertions_on_constants)]
    fn a_host_test_build_carries_no_provisioning_anchor() {
        // Host tests build without `production`, so there is no anchor and the
        // path is inert. If this ever fails, some build is embedding a
        // provisioning key by default -- which is the shape #869 removed.
        assert_eq!(
            PROVISION_PUBLIC_KEY, None,
            "a non-production build must trust no provisioning authority"
        );
        assert!(!PROVISION_KEY_IS_PRODUCTION);
    }
    ...
    // -----------------------------------------------------------------------
    // Signature-verification mechanics (historical #270).
    // -----------------------------------------------------------------------
