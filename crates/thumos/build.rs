...
fn main() {
    ...
    emit_kernel_window(&manifest_dir, &out_dir);
...
/// Emit the kernel window's upper bound as a linker script fragment (#917).
///
/// WHY this is generated rather than written into `link.ld`: the bound is one
/// fact -- `board::KERNEL_LOAD + board::KERNEL_RESERVED` -- and the linker
/// cannot read a Rust constant. Restating it leaves two copies whose drift is
/// silent in the direction that matters: a larger linker bound accepts an image
/// extending past the last L1 section `mmu` maps, and the first report of that
/// is a data abort on device, not a build failure.
///
/// So `board/mod.rs` stays the definition and this derives from it. A parse
/// failure here dies rather than falling back to a literal, because a default
/// would be exactly the second copy this exists to remove.
fn emit_kernel_window(manifest_dir: &Path, out_dir: &Path) {
    const SECTION: u64 = 1 << 20;

    let board_rs = manifest_dir.join("src/board/mod.rs");
    println!("cargo:rerun-if-changed={}", board_rs.display());

    let src = fs::read_to_string(&board_rs)
        .unwrap_or_else(|e| die(&format!("#917: cannot read {}: {e}", board_rs.display())));

    let read_const = |name: &str| -> u64 {
        let needle = format!("const {name}: usize = ");
        let start = src.find(&needle).unwrap_or_else(|| {
            die(&format!(
                "#917: {name} not found in board/mod.rs -- the linker's window bound is \
                 derived from it, so this build cannot produce a correct link script"
            ))
        }) + needle.len();
        let rest = &src[start..];
        let end = rest.find(';').unwrap_or_else(|| {
            die(&format!(
                "#917: {name} in board/mod.rs has no terminating semicolon"
            ))
        });
        let literal: String = rest[..end].trim().chars().filter(|c| *c != '_').collect();
        let digits = literal.strip_prefix("0x").unwrap_or_else(|| {
            die(&format!(
                "#917: {name} in board/mod.rs is `{literal}`, not a hex literal this parser \
                 understands -- keep it as 0x... or teach build.rs the new form"
            ))
        });
        u64::from_str_radix(digits, 16).unwrap_or_else(|e| {
            die(&format!(
                "#917: {name} in board/mod.rs is not valid hex: {e}"
            ))
        })
    };

    let kernel_end = read_const("KERNEL_LOAD") + read_const("KERNEL_RESERVED");
    let ram_start = read_const("RAM_START");
    if (kernel_end - ram_start) % SECTION != 0 {
        die(&format!(
            "#917: the kernel window [{ram_start:#x}, {kernel_end:#x}) is not a whole number \
             of 1 MB sections. Coarse L2 tables cannot map the remainder, so the top of the \
             window would be unmapped rather than rejected."
        ));
    }

    let fragment = out_dir.join("kernel_window.ld");
    fs::write(
        &fragment,
        format!(
            "/* Generated by build.rs from board::KERNEL_LOAD + board::KERNEL_RESERVED (#917). \
             Do not edit; edit board/mod.rs. */\n__kernel_end = {kernel_end:#x};\n"
        ),
    )
    .unwrap_or_else(|e| die(&format!("#917: cannot write {}: {e}", fragment.display())));
