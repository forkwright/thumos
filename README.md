...
- **Aletheia live runtime** ([docs/ALETHEIA-BRIDGE.md](docs/ALETHEIA-BRIDGE.md), `metaxu`): grant verification (Ed25519-signed, expiring, identity-bound) and its adversarial witness are implemented and tested; an on-device transport is proven in QEMU (a real userspace process drives an authenticated round trip over a second UART to a host reference endpoint, `scripts/witness/metaxu.sh`, #544) but a live Aletheia-side endpoint, real grant provisioning, local confirmation/policy UI, and hardware transport (WiFi on the M7) are still future work.
